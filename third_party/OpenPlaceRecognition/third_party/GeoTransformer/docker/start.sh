...
docker run -it -d --rm \
    $ARGS \
    --privileged \
    --name ${USER}_geotransformer \
    --net host \
    -v $PROJECT_ROOT_DIR:/home/docker_geotransformer/GeoTransformer:rw \
    -v $DATASETS_DIR:/home/docker_geotransformer/Datasets:rw \
    geotransformer:$DEVICE-$USER

docker exec --user root \
    ${USER}_geotransformer bash -c "/etc/init.d/ssh start"
